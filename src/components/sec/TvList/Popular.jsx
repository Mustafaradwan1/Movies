...
useEffect(()=>{
  ...
  return <>
    <p className='mt-20 text-2xl mb-4'>Popular</p>
    <div className='cards flex flex-wrap'>
      {Movies.map((movie)=>(
            <Link to={`/Tv/${movie.id}`} className=' card lg:w-1/4 md:w-1/3 sm:w-1/2  p-2' key={movie.id}>
                <div className='box cursor-pointer relative h-96'>
                  ...
                  <div className='absolute card-text h-full  hidden text-white p-5  w-full z-10 top-0 flex-col justify-end'>
                    ...
                    <h2 className=''>{movie.first_air_date}</h2>
