...
const SingleTv = () => {
  ...
  <div className='flex lg:flex-row-reverse flex-wrap lg:px-32 md:px-20 sm:px-10 px-5 py-10 relative z-10'>
        ...
        <div className='SingleText px-5  sm:flex-1 w-full text-white'>
            <h2 className='text-xl mb-10'>{Single.name}</h2>
            <p className='text-xl mb-5'>overview : <span className='text-sm text-slate-400'>{Single.overview}</span></p>
            <div className='flex flex-wrap '>
                <button className='bg-red-500 mt-4 mr-2 py-2 px-5 rounded-lg text-sm  '>genre</button>
            {Single.genres?.map((btn)=>(
                <button className='bg-black mt-4  mr-2 py-2  px-5 rounded-lg text-sm  hover:text-slate-400' key={btn.id}>{btn.name}</button>
                ))}
            </div>
            <p>{Single.release_date}</p>
            <p className='text-xl flex items-center  mt-4'> vote_average : <span className='text-sm ml-5 flex items-center gap-2'>{Math.round(Single.vote_average)} <FaStar/> <FaStar/> <FaStar/></span></p>
        </div>
        </div>
